...
export default function App({ user }) {
   ...
   useEffect(() => {
      ...
      async function getData(r) {
         const data = (
            await axios.post(
               `${process.env.REACT_APP_SERVER}/messenger/getlistchat`,
               { data: r.data() }
            )
         ).data.data;
         if (mount.current) {
      ...
      const docRef = doc(firestore, `users/${user.uid}/messengerlist/data`);
   ...
   useEffect(() => {
      ...
      return () => {
         axios.post(`${process.env.REACT_APP_SERVER}/user/setonline`, {
            useruid: user.uid,
            online: false,
         });
